...
public:
    ...
    bool IsBST(Node *root, int curr_min, int curr_max)
    {
        if (!root)
            return true;
        return root->data > curr_min and root->data < curr_max and IsBST(root->left, curr_min, root->data) and IsBST(root->right, root->data, curr_max);
    }
    ...
    Node *lca2Nodes(Node *root, int n1, int n2)
    {
        ...
        if (root->data >= n1 and root->data <= n2)
            ...
            return lca2Nodes(root->right, n1, n2);
        else
            return lca2Nodes(root->left, n1, n2);
    }
    ...
    {
        if (ind >= pre.size() or pre[ind] > bound)
            return NULL;
    ...
    vector<int> merge2BST(Node *root1, Node *root2)
    {
        ...
        if (i2 >= io2.size())
        {
            for (int i = i1; i < io1.size(); i++)
                ans.push_back(io1[i]);
        }
        else
        {
            for (int i = i2; i < io2.size(); i++)
                ans.push_back(io2[i]);
        }
        return ans;
    }
    ...
    {
        if (pre_ind >= size or pre[pre_ind] > bound)
            return NULL;
    ...
    bool deadEnd(Node *root, int lbound, int ubound)
    {
        if (!root)
            return false;
        if (isLeaf(root) and lbound == ubound)
            return true;
        if (deadEnd(root->left, lbound, root->data - 1))
            return true;
        if (deadEnd(root->right, root->data + 1, ubound))
            return true;
        return false;
    }
